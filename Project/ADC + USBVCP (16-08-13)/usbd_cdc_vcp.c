...
#define BIG_BUF_Len 100
uint8_t BIG_BUF[BIG_BUF_Len] = {0};
uint8_t BIG_BUF_Ptr = 0;
...
static uint16_t VCP_DataRx (uint8_t* Buf, uint32_t Len)
{
  // Save incoming byte into BIG_BUF (leo)
  /*BIG_BUF[BIG_BUF_Ptr++] = *Buf;
  if (BIG_BUF_Ptr == BIG_BUF_Len) BIG_BUF_Ptr = 0;*/
  /*Come faccio io: riempio un buffer che poi vado a svuotare nel main*/
  if (Len >= BIG_BUF_Len) Len = BIG_BUF_Len;
  while (BIG_BUF_Ptr < Len)
  {
    BIG_BUF[BIG_BUF_Ptr] = Buf[BIG_BUF_Ptr];
    BIG_BUF_Ptr++;
  }
 
  
  // Add 1 to the byte and send it back
  //APP_Rx_Buffer[APP_Rx_ptr_in] = (*Buf + 1);
  //APP_Rx_ptr_in++;
