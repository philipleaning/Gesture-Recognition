...

{
  ...
  */  
  ...
  { 
    ...
    /*MHP voltage/current setting*/
    if (set_array.temperature == 1)
    {
      set_array.temperature = 0;
    }
    
    if (set_array.current == 1)
    {
      set_array.current = 0;
    }
    if (set_array.voltage == 1)
    {
      SetVoltageDAC_Ch1(host.voltage);
      set_array.voltage = 0;
    }
...
{
  /*voltage is in mV*/
  /*convert desidered MHP voltage in actual DAC output voltage*/
  int DAC_code;
  //float v_mhp = (float)(voltage)/1000;
  float v_mhp = (float)(voltage);
  //float DAC_V =1000*(0.4 - (v_mhp/7));
  ///float DAC_V =1000*(0.4 - (v_mhp/6.9));
  
  float DAC_V = 2662.5 - (v_mhp/1.24);
  
  //float DAC_V = (float)voltage;
  if (DAC_V < 0) DAC_V = 0;
  DAC_code = (int)(DAC_V * 0xFFF/3000);
  //DAC_SetChannel2Data(DAC_Align_12b_R, DAC_code);
